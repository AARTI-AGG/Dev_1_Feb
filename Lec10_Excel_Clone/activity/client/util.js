function solve(formula, selfCellObject){
 // selfcellobject mtlb b1 (if A1 + A2 )
    // "( 10 + 20 )" bina space ke nhi chlega
    ...
    for(let i=0 ; i<formulaComp.length ; i++){
        ...
        if(fComp[0] >= "A" && fComp[0] <= "Z" ){
            ...
            //DONT WANT TO MAKE DUPLICATE CHILDRENS IF SELF OBJECT AYA TOH 
            if(selfCellObject){
                // push yourself into parent's childrens
                cellObject.childrens.push( selfCellObject.name );
                // push parents into your parents
...
//deleting formula-- parents null or parents ke children meh se htado
...
//updating childrens (basically telling cells on which , which cells are dependents)
