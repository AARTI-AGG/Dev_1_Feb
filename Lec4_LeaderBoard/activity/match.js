...
function cb(error, response, data) {
  ...
  processHtml(data);
  //if all 60 links get completed print the leaderboard(table format)
  if(count == 0){
    console.table(lb);
  }
}
...
function processLeaderBoard(teamName,batsmanName ,runs,balls,fours,sixes)
{
  ...
  //it will go after first entry
  for(let i=0 ; i<lb.length ; i++){
    let inning = lb[i];
    //will check if the batsman already exists if yes then update
